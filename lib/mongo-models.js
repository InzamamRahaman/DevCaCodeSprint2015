...
function MongooseModels(mongoose) {
    ...
    this.writeNewRequest = function(long, lat) {
        ...
        var req = new that.Request({'longitude' : long, 'latitude' : lat});
    ...
    this.writeNewPickup = function(long, lat) {
        var req = new that.Pickup({'longitude' : long, 'latitude' : lat});
        req.save(config.log_mongo_insert_error)
    }

    this.writeNewDropoff = function(long, lat) {
        var req = new that.Dropoff({'longitude' : long, 'latitude' : lat});
        req.save(config.log_mongo_insert_error)
    ...
    this.getPickupLocations = function(callback) {
        that.Pickup.find({}, function(err, res) {
           callback(err, res);
        });
    }

    this.getDropoffLocations = function(callback) {
        that.Dropoff.find({}, function(err, res) {
           callback(err, res);
        });
    }
